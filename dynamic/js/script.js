  ...
  if (contactForm) {
    ...
    contactForm.addEventListener("submit", function (e) {
      ...
      if (!valid) {
        ...
        feedback.classList.add("is-visible", "error");
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener("submit", function (e) {
      ...
      if (cart.length === 0) {
        e.preventDefault();
        window.alert(
          "Your order is empty. Please add items from the menu first.",
        );
        return;
      }
      var cartField = checkoutForm.querySelector('[name="cart_data"]');
      if (cartField) {
